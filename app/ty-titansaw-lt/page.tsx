...
export default function ThankYouPage() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && !alreadyTracked) {
      ...
      const ecValue = parseFloat(sessionStorage.getItem('ec_value') || '64');
      ...
      script.onload = () => {
        ...
        if (ecAddress) userData.address = { street: ecAddress, country: 'LT' };
        if (Object.keys(userData).length > 0) window.gtag('set', 'user_data', userData);

        // Purchase LT conversion - UPDATE CONVERSION LABEL WHEN PROVIDED
        window.gtag('event', 'conversion', {
          'send_to': 'AW-17261661993/TITANSAW_LT_CONVERSION_LABEL',
          'value': ecValue,
          'currency': 'EUR',
    ...
    <div className="ty-container" style={{
      ...
      <div className="ty-box" style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <div>
              ...
              <div style={{ color: '#64748b', fontSize: '0.85rem' }}>Mokėsite grynaisiais kurjeriui</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <div>
              ...
              <div style={{ color: '#64748b', fontSize: '0.85rem' }}>Pristatymas per 24-48 val.</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <div>
              <div style={{ fontWeight: 600, color: '#1E293B', fontSize: '0.95rem' }}>30 dienų grąžinimas</div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '0.85rem', color: '#64748b', marginBottom: '0.25rem' }}>Reikia pagalbos arba norite grąžinti?</div>
          <a href="mailto:info@kizbuy.com" style={{ color: '#FF8C00', fontWeight: 600, textDecoration: 'none', fontSize: '0.95rem' }}>info@kizbuy.com</a>
          <div style={{ fontSize: '0.8rem', color: '#94a3b8', marginTop: '0.5rem' }}>Galite prašyti pinigų grąžinimo per 30 dienų, jei nesate patenkinti produktu</div>
        </div>
