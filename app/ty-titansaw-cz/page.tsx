...
export default function ThankYouPage() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && !alreadyTracked) {
      ...
      const ecValue = parseFloat(sessionStorage.getItem('ec_value') || '1799');
      ...
      script.onload = () => {
        ...
        if (ecAddress) userData.address = { street: ecAddress, country: 'CZ' };
        if (Object.keys(userData).length > 0) window.gtag('set', 'user_data', userData);

        // Purchase CZ conversion - UPDATE CONVERSION LABEL WHEN PROVIDED
        window.gtag('event', 'conversion', {
          'send_to': 'AW-17261661993/TITANSAW_CZ_CONVERSION_LABEL',
          'value': ecValue,
          'currency': 'CZK',
      ...
      <div className="ty-box" style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <div>
              <div style={{ fontWeight: 600, color: '#1E293B', fontSize: '0.95rem' }}>Platba při doručení</div>
              <div style={{ color: '#64748b', fontSize: '0.85rem' }}>Platíte hotově kurýrovi</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '0.8rem', color: '#94a3b8', marginTop: '0.5rem' }}>Můžete požádat o vrácení peněz do 30 dnů, pokud nejste s produktem spokojeni</div>
        </div>
