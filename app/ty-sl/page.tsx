...
export default function ThankYouPage() {
  ...
  useEffect(() => {
    ...
    if (skipConversion === 'true') { sessionStorage.removeItem('skipConversion'); return; }
    if (typeof window !== 'undefined' && !alreadyTracked) {
      ...
      script.onload = () => {
        ...
        if (ecAddress) userData.address = { street: ecAddress, country: 'SI' };
        if (Object.keys(userData).length > 0) window.gtag('set', 'user_data', userData);
        window.gtag('event', 'conversion', { 'send_to': 'AW-17261661993/AwqOCN3dv-EbEKmegKdA', 'value': ecValue, 'currency': 'EUR', 'transaction_id': transactionId });
  ...
  return (
    <div className="ty-container" style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1E293B 0%, #0F172A 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
