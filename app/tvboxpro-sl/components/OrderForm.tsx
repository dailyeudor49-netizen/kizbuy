      ...
      return (
          <section id="order-form" className="py-16 px-4 bg-green-50 min-h-[50vh] flex items-center justify-center">
              <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-2xl text-center border-4 border-green-500">
                  ...
                  <h2 className="text-3xl font-black text-gray-900 mb-4">HVALA, {formState.name}!</h2>
                  <p className="text-xl text-gray-700 mb-6">Vase narocilo je bilo potrjeno.</p>
                  <p className="text-gray-600">Nas sodelavec vas bo kmalu kontaktiral na stevilko <span className="font-bold">{formState.phone}</span>.</p>
                  <div className="mt-6 bg-yellow-100 p-4 rounded-lg border border-yellow-300">
                    <p className="font-bold text-gray-800 uppercase text-sm mb-1">Znesek za placilo kurirju:</p>
    ...
    <section id="order-form" className="py-12 px-4 bg-gray-200">
      <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden border-2 border-gray-300">
        <div className="bg-red-600 text-white p-6 text-center">
          <h2 className="text-3xl font-black uppercase mb-2">OBRAZEC ZA NAROCILO</h2>
          <p className="text-lg font-medium">Izpolnite spodaj, da si zagotovite akcijo</p>
          <div className="flex items-center justify-center gap-2 mt-4 bg-red-700 py-2 rounded-lg border border-red-500">
            ...
            <span className="font-bold text-xl">Ponudba se konca cez: {formatTime(timeLeft)}</span>
          </div>
        </div>
        <div className="p-6 md:p-8">
          <div className="bg-yellow-50 border-2 border-yellow-400 p-4 rounded-lg mb-8 shadow-sm">
            <h3 className="font-bold text-gray-800 text-lg mb-2 border-b border-yellow-200 pb-2">Pregled narocila:</h3>
            ...
            <div className="flex justify-between items-center text-gray-700 text-sm mb-2">
              <span className="flex items-center gap-1"><Truck size={16}/> Hitra dostava (24/48h)</span>
              <span className="font-bold text-green-600 flex items-center gap-1">AKTIVNO <Check size={16} strokeWidth={3} /></span>
            </div>
            <div className="flex justify-between items-center mt-3 pt-3 border-t-2 border-yellow-200 text-2xl font-black text-red-600">
              <span>SKUPAJ (Placilo ob dostavi):</span>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="block text-gray-700 font-bold mb-2 text-lg">Ime in Priimek <span className="text-red-500">*</span></label>
              <input type="text" name="name" required placeholder="Npr. Janez Novak" className="w-full p-4 border-2 border-gray-300 rounded-lg text-lg focus:border-green-500 focus:ring-green-500 outline-none" value={formState.name} onChange={handleChange} />
            </div>
            <div>
              <label className="block text-gray-700 font-bold mb-2 text-lg">Telefonska stevilka <span className="text-red-500">*</span></label>
              <input type="tel" name="phone" required placeholder="Npr. +386 31 234 567" className="w-full p-4 border-2 border-gray-300 rounded-lg text-lg focus:border-green-500 focus:ring-green-500 outline-none" value={formState.phone} onChange={handleChange} />
              <p className="text-sm text-gray-500 mt-1 font-medium bg-blue-50 p-2 rounded text-blue-800">Pomembno: Kurir bo poklical na to stevilko pred dostavo.</p>
            </div>
            <div>
              <label className="block text-gray-700 font-bold mb-2 text-lg">Popoln naslov dostave <span className="text-red-500">*</span></label>
              <textarea name="address" required placeholder="Mesto, Ulica, Hisna stevilka, Stanovanje (ali prevzemno mesto)" rows={3} className="w-full p-4 border-2 border-gray-300 rounded-lg text-lg focus:border-green-500 focus:ring-green-500 outline-none resize-none" value={formState.address} onChange={handleChange}></textarea>
            </div>
            <div className="bg-gray-100 p-4 rounded-lg flex items-center gap-3 text-sm text-gray-600 border border-gray-200">
               <Lock size={20} className="text-green-600"/>
               <span>Brez tveganja: placate z gotovino/kartico sele ko prejmete posiljko.</span>
            </div>
            <button type="submit" disabled={isSubmitting} className={`w-full text-white text-2xl md:text-3xl font-black py-6 px-6 rounded-xl shadow-xl transform transition border-b-8 whitespace-normal leading-tight ${isSubmitting ? 'bg-gray-400 border-gray-500 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700 hover:scale-105 border-green-800 animate-pulse-btn'}`}>
              {isSubmitting ? 'POSILJAM...' : 'POTRDI NAROCILO'}
              <span className="block text-lg font-normal mt-2 text-green-100">Hitra Dostava in Placilo ob Dostavi</span>
            </button>
          </form>
        </div>
      </div>
